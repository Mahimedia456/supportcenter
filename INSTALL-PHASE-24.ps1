...
Write-Host "Support Command Center - Phase 24 Snapshot-only + Chunked Sync + SLA" -ForegroundColor Cyan
...
$Home = Join-Path $Mobile "src\app\(tabs)\index.tsx"
$HomeText = [IO.File]::ReadAllText($Home)

foreach ($Term in @("useGlobalSupportSnapshot","SLA Health","Breached","No reply > 4h","First reply > 4h","Unsolved > 72h")) {
    if ($HomeText -notmatch [regex]::Escape($Term)) { throw "Home missing: $Term" }
}

foreach ($Forbidden in @("zendeskRecentTickets","zendeskHealth","forceZendeskDbSync","loadZendeskTicketUniverse")) {
    if ($HomeText -match [regex]::Escape($Forbidden)) { throw "Home still calls live Zendesk source: $Forbidden" }
}
...
$ProfileText = [IO.File]::ReadAllText($Profile)

foreach ($Term in @("Sync Support Data","syncZendeskDb","Sync in progress","seedSupportSnapshot")) {
    if ($ProfileText -notmatch [regex]::Escape($Term)) { throw "Profile sync missing: $Term" }
}
...
Write-Host ""
Write-Host "PASS: Phase 24 installed and verified." -ForegroundColor Green
Write-Host ""
Write-Host "NEXT:" -ForegroundColor Cyan
Write-Host "1. git add . && git commit && git push"
Write-Host "2. Wait for Vercel deployment"
Write-Host "3. GitHub Actions -> Hourly Zendesk Snapshot Sync -> Run workflow"
Write-Host "4. Expect separate short stage/chunk calls instead of one 5-minute Vercel function"
